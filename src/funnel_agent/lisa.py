...
def _send_followup_sms(pool: ConnectionPool, settings: Settings, call_id: str, to_number: str | None,
                       name: str | None) -> bool:
    """Fire the MINIMAL curiosity SMS on a missed call via RETELL (create-sms-chat) — a two-way SMS from
    Lisa's SMS-enabled number (827). The message is generated by the SMS chat agent (a tiny 'it's Lisa,
    call me back' prompt), and if the prospect texts back, the agent replies. Gated on LISA_SMS_ENABLED +
    LISA_SMS_AGENT_ID; no-ops (logged) otherwise."""
    if not to_number or not getattr(settings, "lisa_sms_enabled", False):
        return False
    sms_from = getattr(settings, "lisa_sms_from", "") or ""
    sms_agent = getattr(settings, "lisa_sms_agent_id", "") or ""
    if not (sms_from and sms_agent):
    ...
    try:
        _retell(settings, "POST", "create-sms-chat", {
            "from_number": sms_from, "to_number": _e164_au(to_number), "override_agent_id": sms_agent,
            "retell_llm_dynamic_variables": {"prospect_name": first},
            "metadata": {"lisa_call_id": call_id}})
    except Exception as exc:
        log.warning("lisa_sms_failed", call_id=call_id, error=str(exc)[:160])
        return False
    with pool.connection() as conn, conn.cursor() as cur:
